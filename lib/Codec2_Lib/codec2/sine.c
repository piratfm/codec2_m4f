...
void make_analysis_window(float w[],COMP W[])
{
  ...
  */
  ...
  m = 0.0;
  arm_fill_f32(0.0, w, M_PAF/2-NW/2);
  for(i=M_PAF/2-NW/2,j=0; i<M_PAF/2+NW/2; i++,j++) {
    w[i] = 0.5 - 0.5*arm_cos_f32(TWO_PI*j/(NW-1));
    m += w[i]*w[i];
  }
  arm_fill_f32(0.0, &w[M_PAF/2+NW/2], M_PAF - (M_PAF/2+NW/2));
 
  /* Normalise - makes freq domain amplitude estimation straight
     forward */

  arm_sqrt_f32(m*FFT_ENC, &m);
  m = 1.0/m;
  arm_scale_f32 (w, m, w, M_PAF);
  ...
  */
  ...
  arm_fill_f32(0.0, &W[0].real, FFT_ENC*2);

  arm_2copy_f32(&w[M_PAF/2], &W[0].real, NW/2);
  arm_2copy_f32(&w[M_PAF/2-NW/2], &W[FFT_ENC-NW/2].real, NW/2);
...
void dft_speech(COMP Sw[], float Sn[], float w[])
{
  ...
  arm_fill_f32(0.0, &Sw[0].real, FFT_ENC*2);

  /* Centre analysis window on time axis, we need to arrange input
     to FFT this way to make FFT phases correct */
  
  /* move 2nd half to start of FFT input vector */
  arm_2mult_f32(&Sn[M_PAF/2], &w[M_PAF/2], &Sw[0].real, NW/2);

  /* move 1st half to end of FFT input vector */
  arm_2mult_f32(&Sn[M_PAF/2-NW/2], &w[M_PAF/2-NW/2], &Sw[FFT_ENC-NW/2].real, NW/2);
    ...
    float prev_Wo)
{
    ...
    arm_fill_f32(0.0, &Sw_[0].real, FFT_ENC*2);
    arm_fill_f32(0.0, &Ew[0].real, FFT_ENC*2);
...
void make_synthesis_window(float Pn[])
{
  ...
  arm_fill_f32(0.0, Pn, N_SPF/2-TW);

  win = 0.0;
  for(i=N_SPF/2-TW; i<N_SPF/2+TW; win+=1.0/(2*TW), i++ )
    Pn[i] = win;

  assert((3*N_SPF/2-TW) - N_SPF/2+TW >= 0);
  arm_fill_f32(1.0, &Pn[N_SPF/2+TW], (3*N_SPF/2-TW) - N_SPF/2+TW);

  win = 1.0;
  for(i=3*N_SPF/2-TW; i<3*N_SPF/2+TW; win-=1.0/(2*TW), i++)
    Pn[i] = win;

  assert((3*N_SPF/2+TW) - 2*N_SPF >= 0);
  arm_fill_f32(0.0, &Pn[3*N_SPF/2+TW], (3*N_SPF/2+TW) - 2*N_SPF);
}
...
)
{
	...
	arm_copy_f32(&Sn_[N_SPF], Sn_, N_SPF-1);
	Sn_[N_SPF-1] = 0.0;
    }

    arm_fill_f32(0.0, &Sw_[0].real, FFT_DEC*2);
...
#define FFT_SYNTHESIS
	...
	Sw_[b].real = model->A[l]*arm_cos_f32(model->phi[l]);
	Sw_[b].imag = model->A[l]*arm_sin_f32(model->phi[l]);
    ...
    */
    for(l=1; l<=model->L; l++) {
	for(i=0,j=-N_SPF+1; i<N_SPF-1; i++,j++) {
	    Sw_[FFT_DEC-N_SPF+1+i].real += 2.0*model->A[l]*arm_cos_f32(j*model->Wo*l + model->phi[l]);
	}
 	for(i=N_SPF-1,j=0; i<2*N_SPF; i++,j++)
	    Sw_[j].real += 2.0*model->A[l]*arm_cos_f32(j*model->Wo*l + model->phi[l]);
    }	
#endif

    /* Overlap add to previous samples */
    for(i=0; i<N_SPF-1; i++) {
	Sn_[i] += Sw_[FFT_DEC-N_SPF+1+i].real*Pn[i];
    }
