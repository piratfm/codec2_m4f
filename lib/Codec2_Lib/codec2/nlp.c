...
typedef struct {
    ...
    float mem_fir[NLP_NTAP+N_SPF-1]; /* decimation FIR filter memory */
...
void *nlp_create()
{
    ...
    arm_fill_f32 (0.0, nlp->sq, PMAX_M);
    nlp->mem_x = 0.0;
    nlp->mem_y = 0.0;
    arm_fill_f32 (0.0, nlp->mem_fir, NLP_NTAP+N_SPF-1);
    arm_fir_init_f32 (&nlp->arm_fir, NLP_NTAP, (float32_t *) nlp_fir, nlp->mem_fir, N_SPF);
...
)
{
    ...
    float  FwR[PE_FFT_SIZE];	/* DFT of squared signal */
    float  gmax;
    uint32_t    gmax_bin;
    int   i,j;
    float best_f0;
    ...
    /* square latest speech samples */
    arm_mult_f32 (&Sn[m-n], &Sn[m-n], &nlp->sq[m-n], M_PAF - (m-n));
    ...
    assert(n==N_SPF);
    /* FIR filter vector */
    arm_fir_f32 (&nlp->arm_fir, &nlp->sq[m-n], &nlp->sq[m-n], n);
    /* Decimate and DFT */

    arm_fill_f32 (0.0, &Fw[0].real, PE_FFT_SIZE*2);
    for(i=0; i<m/DEC; i++) {
	Fw[i].real = nlp->sq[i*DEC]*(0.5 - 0.5*arm_cos_f32(2*PI*i/(m/DEC-1)));
    }
#ifdef DUMP
    dump_dec(Fw);
#endif
    fft(&Fw[0].real,PE_FFT_SIZE,1);
    arm_cmplx_mag_squared_f32 ((float32_t *) &Fw[0].real, FwR, PE_FFT_SIZE);
    ...
    /* find global peak */
    arm_max_f32 (&FwR[PE_FFT_SIZE*DEC/pmax],
    		      PE_FFT_SIZE*DEC/pmin - PE_FFT_SIZE*DEC/pmax + 1,
    		      &gmax, &gmax_bin);
    gmax_bin += PE_FFT_SIZE*DEC/pmax;

    best_f0 = post_process_sub_multiples(FwR, pmin, pmax, gmax, gmax_bin,
					 prev_Wo);

    /* Shift samples in buffer to make room for new samples */

    arm_copy_f32 (&nlp->sq[n], nlp->sq, m-n);
