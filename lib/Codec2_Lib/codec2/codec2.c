...
struct CODEC2 {
    int    mode;
    float  w[M_PAF];	        /* time domain hamming window                */
    COMP   W[FFT_ENC];	        /* DFT of w[]                                */
    float  Pn[2*N_SPF];	        /* trapezoidal synthesis window              */
    float  Sn[M_PAF];               /* input speech                              */
    float  hpf_states[2];       /* high pass filter states                   */
    void  *nlp;                 /* pitch predictor states                    */
    float  Sn_[2*N_SPF];	        /* synthesised output speech                 */
...
struct CODEC2 *codec2_create(int mode)
{
    ...
    c2->mode = mode;
    arm_fill_f32 (1.0, c2->Sn, M_PAF);
    c2->hpf_states[0] = c2->hpf_states[1] = 0.0;
    arm_fill_f32 (0.0, c2->Sn_, 2*N_SPF);
    ...
    arm_fill_f32 (0.0, &c2->prev_model.A[1], MAX_AMP-1);
...
void codec2_encode_2500(struct CODEC2 *c2, unsigned char * bits, short speech[])
{
    ...
    analyse_one_frame(c2, &model, &speech[N_SPF]);
...
void codec2_decode_2500(struct CODEC2 *c2, short speech[], const unsigned char * bits)
{
    ...
    synthesise_one_frame(c2, &speech[N_SPF], &model, ak);
...
void codec2_encode_1500(struct CODEC2 *c2, unsigned char * bits, short speech[])
{
    ...
    */
    ...
    //fprintf(stderr,"frame: %d\n", ++frames);
    analyse_one_frame(c2, &model, &speech[2*N_SPF]);
    voiced3 = model.voiced;

    /* frame 4: - voicing and delta Wo -----------------------------  */

    //fprintf(stderr,"frame: %d\n", ++frames);
    analyse_one_frame(c2, &model, &speech[3*N_SPF]);
...
void codec2_decode_1500(struct CODEC2 *c2, short speech[], const unsigned char * bits)
{
    ...
    */
    ...
    synthesise_one_frame(c2, &speech[2*N_SPF], &model_interp, ak_interp);
    //fprintf(stderr,"  buf[0] %d\n", speech[2*N]);
    synthesise_one_frame(c2, &speech[3*N_SPF], &model, ak);
    //fprintf(stderr,"  buf[0] %d\n", speech[3*N]);
...
void analyse_one_frame(struct CODEC2 *c2, MODEL *model, short speech[])
{
    ...
    arm_copy_f32 (&c2->Sn[N_SPF], c2->Sn, M_PAF-N_SPF);
    for(i=0; i<N_SPF; i++)
      c2->Sn[i+M_PAF-N_SPF] = speech[i];

    dft_speech(Sw, c2->Sn, c2->w);

    /* Estimate pitch */

    nlp(c2->nlp,c2->Sn,N_SPF,M_PAF,P_MIN,P_MAX,&pitch,Sw, &c2->prev_Wo);
