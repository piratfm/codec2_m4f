...
)
{
  ...
  arm_fill_f32 (0.0, &Pw[0].real, FFT_DEC*2);
  ...
  for(m=1; m<=model->L; m++) {
    ...
    //arm_cmplx_ersq_f32(&Pw[am].real, G, bm-am, &Am);

    phi_ = -atan2(Pw[b].imag,Pw[b].real);
    H[m].real = Am*arm_cos_f32(phi_);
    H[m].imag = Am*arm_sin_f32(phi_);
  }
}
  ...
  */
  ...
  for(m=1; m<=model->L; m++) {
	...
	*/
	...
	Ex[m].real = arm_cos_f32(ex_phase[0]*m - jitter*model->Wo*m + glottal[b]);
	Ex[m].imag = arm_sin_f32(ex_phase[0]*m - jitter*model->Wo*m + glottal[b]);
    }
    else {
	...
	Ex[m].real = arm_cos_f32(phi);
	Ex[m].imag = arm_sin_f32(phi);
    }
