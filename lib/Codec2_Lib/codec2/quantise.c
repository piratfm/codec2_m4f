...
)
{
  ...
  arm_mult_f32 (Sn, w, Wn, M_PAF);
  autocorrelate(Wn,R,M_PAF,order);
...
)
{
  ...
  float PwR[FFT_DEC];	/* power spectrum */
  ...
  arm_cmplx_pwr_f32(&Pw[0].real, PwR, E, FFT_DEC/2);
#ifdef DUMP
  if (dump) 
      dump_Pw(Pw);
#endif
